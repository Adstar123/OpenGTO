...
class GTOTrainer:
    ...
    def _save_checkpoint(self, final: bool = False):
        ...
        torch.save({
            ...
            'curriculum_stage_iterations': self.curriculum.stage_iterations if self.curriculum else None,
        }, path)
    ...
    def load_checkpoint(self, path: str):
        ...
        if 'curriculum_stage' in checkpoint and checkpoint['curriculum_stage'] is not None:
            if self.curriculum:
                ...
                # Restore stage iterations
                if 'curriculum_stage_iterations' in checkpoint:
                    self.curriculum.stage_iterations = checkpoint['curriculum_stage_iterations']
                else:
                    # Calculate based on iteration and curriculum progress
                    completed_iters = sum(
                        s.iterations for s in self.curriculum.stages[:self.curriculum.current_stage_idx]
                    )
                    self.curriculum.stage_iterations = self.iteration - completed_iters
                self.curriculum.total_iterations = self.iteration
                # Update engine for current stage
                self._update_engine_for_curriculum()
