    ...
    def _build_scenario(
        ...
        if action_history:
            ...
            for pos in preflop_order:
                ...
                # Random action: 70% fold, 20% raise, 10% call (if applicable)
                r = random.random()
                if r < 0.70:
                    builder.add_action(pos, Action(ActionType.FOLD))
                    villain_actions.append(f"{pos.name} folds")
                elif r < 0.90:
                    raise_size = random.choice([2.5, 3.0, 3.5])
                    builder.add_action(pos, Action(ActionType.RAISE, amount=raise_size))
                    villain_actions.append(f"{pos.name} raises to {raise_size}bb")
                    break  # Stop after a raise for simplicity
                # else: would be call, skip for now
