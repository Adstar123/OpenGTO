"""
Trainer with weighted loss function to handle class imbalance.
"""
...
class WeightedPreflopTrainer:
    """Trainer with weighted loss for balanced learning."""
    ...
    def __init__(self, model, learning_rate: float = 0.001, device: str = None):
        ...
        # We'll set up weighted loss in train method
    ...
    def train(self, train_data: List[Dict], val_data: List[Dict], 
        ...
        actions = [sample['optimal_action']['action'] for sample in train_data]
        action_labels = [{'fold': 0, 'call': 1, 'raise': 2}[action] for action in actions]
        
        # Compute class weights
        class_weights = compute_class_weight('balanced', classes=np.unique(action_labels), y=action_labels)
        class_weights_tensor = torch.FloatTensor(class_weights).to(self.device)
        
        self.logger.info(f"Class weights: fold={class_weights[0]:.3f}, call={class_weights[1]:.3f}, raise={class_weights[2]:.3f}")
        ...
        self.logger.info(f"Starting weighted training on {self.device}")
    ...
    def _train_epoch(self, dataloader: DataLoader) -> Tuple[float, float]:
        ...
        for batch_features, batch_actions, batch_sizes in dataloader:
            ...
            # Calculate losses with higher weight on action accuracy
            ...
            # Heavily weight action prediction over sizing
            total_loss_batch = action_loss + 0.1 * sizing_loss
